...
impl SimpleComponent for DemoListModel{
    ...
    fn init(
        ...
        
        {
            ...
            model.list_selection.connect_selection_changed(move |_,_,_| {
        ...
        name_factory.connect_setup(|_, li|{
            ...
            listitem.property_expression("item").chain_property::<DemoObject>("name").bind(&label, "label", gtk::Widget::NONE);
        });
        widgets.demo_list.append_column(&gtk::ColumnViewColumn::builder()
            .title("Name")
            .resizable(true)
            .factory(&name_factory)
            .expand(true)
            .sorter(
                &gtk::StringSorter::new(Some(&
                    gtk::PropertyExpression::new(DemoObject::static_type(), None::<gtk::Expression>,"name")
                ))
            )
            .build());
        ...
        map_factory.connect_setup(|_, li|{
            ...
            let label = gtk::Label::builder().halign(gtk::Align::Start).build();
            listitem.set_child(Some(&label));
            listitem.property_expression("item").chain_property::<DemoObject>("map").bind(&label, "label", gtk::Widget::NONE);
        });
        widgets.demo_list.append_column(&gtk::ColumnViewColumn::builder()
            .title("Map")
            .resizable(true)
            ...
            .sorter(
                &gtk::StringSorter::new(Some(&
                    gtk::PropertyExpression::new(DemoObject::static_type(), None::<gtk::Expression>,"map")
                ))
            )
            .build());
        ...
        username_factory.connect_setup(|_, li|{
            ...
            let label = gtk::Label::builder().halign(gtk::Align::Start).build();
            listitem.set_child(Some(&label));
            listitem.property_expression("item").chain_property::<DemoObject>("username").bind(&label, "label", gtk::Widget::NONE);
        });
        widgets.demo_list.append_column(&gtk::ColumnViewColumn::builder()
            .title("Username")
            .resizable(true)
            ...
            .sorter(
                &gtk::StringSorter::new(Some(&
                    gtk::PropertyExpression::new(DemoObject::static_type(), None::<gtk::Expression>,"username")
                ))
            )
            .build());
        ...
        duration_factory.connect_setup(|_, li|{
            ...
            listitem.property_expression("item").chain_property::<DemoObject>("duration").chain_closure_with_callback(|v|{
                humantime::format_duration(std::time::Duration::from_secs(v[1].get::<f32>().unwrap() as u64)).to_string()
        ...
        widgets.demo_list.append_column(&gtk::ColumnViewColumn::builder()
            ...
            .expand(true)
            .sorter(
                &gtk::NumericSorter::new(Some(&
                    gtk::PropertyExpression::new(DemoObject::static_type(), None::<gtk::Expression>,"duration")
                ))
            )
            .build());
        ...
        let date_column = &gtk::ColumnViewColumn::builder()
            ...
            .sorter(
                &gtk::NumericSorter::new(Some(&
                    gtk::PropertyExpression::new(DemoObject::static_type(), None::<gtk::Expression>,"created")
                ))
            )
            .build();
        ...
        widgets.demo_list.append_column(&gtk::ColumnViewColumn::builder()
            ...
            .sorter(
                &gtk::NumericSorter::new(Some(&
                    gtk::PropertyExpression::new(DemoObject::static_type(), None::<gtk::Expression>,"size")
                ))
            )
            .build());
        ...
        bookmark_factory.connect_setup(|_, li|{
            ...
            let label = gtk::Label::builder().halign(gtk::Align::End).build();
            listitem.set_child(Some(&label));
            listitem.property_expression("item").chain_property::<DemoObject>("bookmarks").bind(&label, "label", gtk::Widget::NONE);
        });
        widgets.demo_list.append_column(&gtk::ColumnViewColumn::builder()
            .title("Bookmarks")
            .resizable(true)
            ...
            .sorter(
                &gtk::NumericSorter::new(Some(&
                    gtk::PropertyExpression::new(DemoObject::static_type(), None::<gtk::Expression>,"bookmarks")
                ))
            )
            .build());

        widgets.demo_list.sort_by_column(Some(date_column), gtk::SortType::Descending);
    ...
    fn update(&mut self, message: Self::Input, sender: ComponentSender<Self>) {
        match message {
            DemoListMsg::Update(demos, scroll) => {
                let model_set: HashSet<(String, u64), RandomState> = HashSet::from_iter(self.list_model.into_iter().map(|d|{
                    let demo = d.unwrap().downcast::<DemoObject>().unwrap();
                    (demo.name(), demo.size())
                }));

                let data_set: HashSet<(String, u64), RandomState> = HashSet::from_iter(demos.iter().map(|t|{
                    let demo = t.1;
                ...
                for dn in added {
                    self.list_model.append(&DemoObject::new(demos.get(&dn.0).unwrap()));
                }
